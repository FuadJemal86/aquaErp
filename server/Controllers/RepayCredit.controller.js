...
const repaySalesCredit = async (req, res) => {
    ...
    try {
        ...
        const existingCredit = await prisma.sales_credit_transaction.findFirst({
            where: { transaction_id },
            orderBy: { createdAt: "desc" },
        });

        if (req.file && existingCredit?.image) {
            const oldPath = path.join(__dirname, "..", existingCredit.image);
        ...
        if (updatedTotal <= 0) {
            await prisma.sales_credit.delete({ where: { id: credit.id } });
        } else {
